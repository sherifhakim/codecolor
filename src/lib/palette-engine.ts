...
export const CHROMATIC_FAMILIES = ["Y", "YR", "R", "RV", "V", "BV", "B", "BG", "G", "YG", "GY"];
...
export const ZONES = {
  ...
  Neon:   (c: ColorItem) => c.family === "FY" || (["RV", "R", "Y", "YR", "G", "BG", "GY"].includes(c.family) && c.s > 65),
  Vintage:(c: ColorItem) => ["E", "V", "BV", "YGY"].includes(c.family) && c.s >= 20 && c.s <= 50,
  Summer: (c: ColorItem) => ["Y", "YR", "R", "RV", "BG", "G", "YG", "GY", "E"].includes(c.family) && c.l >= 50,
  Winter: (c: ColorItem) => {
...
export function getShadowHighlight(color: ColorItem, dataset: ColorItem[]) {
  ...
  // Fallback to lightness diff
  if (!highlight) {
    const lighter = familySiblings.filter(c => c.l > color.l).sort((a,b) => a.l - b.l); // closest lightness > c.l
    if (lighter.length > 0) highlight = lighter[0];
  }
  if (!shadow) {
    const darker = familySiblings.filter(c => c.l < color.l).sort((a,b) => b.l - a.l); // closest lightness < c.l
    if (darker.length > 0) shadow = darker[0];
  }
